...
public class ChartAveragePerZoneController implements Callback<List<XnYDataHolder>> {
    ...
    public ChartAveragePerZoneController(Context context, RetrofitViewModel retrofitViewModel) {
    ...
    public void start(int stateId, String date, int section, String endDate) {
        ...
        Call<List<XnYDataHolder>> call = apiService.GetAveragePerZone(stateId, date, section, endDate);
        call.enqueue(this);
    ...
    public void onResponse(Call<List<XnYDataHolder>> call, Response<List<XnYDataHolder>> response) {
        if(response.isSuccessful()) {
            ...
            if(data.size() > 0){
                retrofitViewModel.setAveragePerZoneData(data);
            } else {
