...
public class ChartTradePerMonthController implements Callback<List<XnYDataHolder>> {
    ...
    public ChartTradePerMonthController(Context context, RetrofitViewModel retrofitViewModel) {
    ...
    public void start(int stateId, String date) {
        ...
        Call<List<XnYDataHolder>> call = apiService.GetTradePerMonth(stateId, date);
        call.enqueue(this);
    ...
    public void onResponse(Call<List<XnYDataHolder>> call, Response<List<XnYDataHolder>> response) {
        if(response.isSuccessful()) {
            ...
            if(data.size() > 0){
                retrofitViewModel.setTradePerMonthData(data);
            } else {
