...
public class ChartAveragePerMonthController implements Callback<List<XnYDataHolder>> {
    ...
    public ChartAveragePerMonthController(Context context, RetrofitViewModel retrofitViewModel) {
    ...
    public void start(int stateId, String date) {
        ...
        Call<List<XnYDataHolder>> call = apiService.GetAveragePerMonth(stateId, date);
        call.enqueue(this);
    ...
    public void onResponse(Call<List<XnYDataHolder>> call, Response<List<XnYDataHolder>> response) {
        if(response.isSuccessful()) {
            ...
            if(data.size() > 0){
                retrofitViewModel.setAveragePerMonthData(data);
            } else {
