...
public class ChartAveragePerSectionController implements Callback<List<XnYDataHolder>> {
    ...
    public ChartAveragePerSectionController(Context context, RetrofitViewModel retrofitViewModel) {
    ...
    public void start(int stateId, String date) {
        ...
        Call<List<XnYDataHolder>> call = apiService.GetAveragePerSection(stateId, date);
        call.enqueue(this);
    ...
    public void onResponse(Call<List<XnYDataHolder>> call, Response<List<XnYDataHolder>> response) {
        if(response.isSuccessful()) {
            ...
            if(data.size() > 0){
                retrofitViewModel.setAveragePerSectionData(data);
            } else {
